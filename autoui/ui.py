...
_last_mouse_move_time = datetime.datetime.now()
_mouse_running_diff = 0
...
def move_mouse(pos, realistic=True, careful=False):
    ...
    for pos in points:
        ...
        for x, y in curve:
            with _mouse_lock:
                ...
                _last_mouse_pos = pyautogui.Point(int(x), int(y))
        time.sleep(random.uniform(0.001, 0.1))

def screenshot(tl=None, br=None):
...
def __on_mouse_move():
    ...
    with _mouse_lock:
        ...
        if pyautogui.position() == _last_mouse_pos:
            return

        # Keep track of how many pixels we've moved per second
        now = datetime.datetime.now()
        diff = (now - _last_mouse_move_time).total_seconds() * 1000
        global _mouse_running_diff
        _mouse_running_diff = max(_mouse_running_diff - diff, 0)

        # If it wasn't us who moved it, calculate difference in position        
        _mouse_running_diff += math.sqrt(
            (pos.x - _last_mouse_pos.x) ** 2 +
            (pos.y - _last_mouse_pos.y) ** 2
        )

        # If the running diff ever goes above the limit, that means the user is
        # trying to move it, and thus, autoui should disengage
        if _mouse_running_diff > 10:
            playsound.playsound('/home/jared/Development/autoui/autoui/assets/disengage.mp3')
            _mouse_running_diff = 0
            _engaged.acquire()

    move_mouse(_last_mouse_pos, realistic=False)

def __check_engagement():
    if not _engaged.locked():
        return
    input("AutoUI has been disengaged. Press 'Enter' to re-engage.")
    _engaged.release()
    print("Resuming control...")


# Initialize module
listener = pynput.mouse.Listener(on_move=__on_mouse_move)
listener.start()